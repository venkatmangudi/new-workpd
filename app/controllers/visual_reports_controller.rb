class VisualReportsController < ApplicationController
	def index
		@employees=Employee.accessible_by(current_ability).order('emp_full_name ASC')
		@hospitals=Hospital.accessible_by(current_ability).all
	...
	def generate_report
		...
		if(@entity == "1")
			...
			@final_data=HospitalPerforma.accessible_by(current_ability).where(:month_id=>@month_range).where(:year=>@year_range).where(:hospital_id=>@hospital)
		elsif(@entity == "2")
			...
			@final_data=Performaone.accessible_by(current_ability).where(:month_id=>@month_range).where(:year=>@year_range).where(:employee_id=>@employee)
		end
